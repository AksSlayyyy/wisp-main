...
function visualReadinessReport() {
  ...
  const coverage = answers.length ? Math.round(answered.length / answers.length * 100) : 0;
  const segments = [
  ...
  return `
    ...
    <div class="readiness-report readiness-report-executive">
      <section class="readiness-coverage" aria-label="Assessment coverage"><div><strong>${coverage}% assessed</strong><span>${answered.length} of ${answers.length} safeguards reviewed${missing ? " · Results are provisional" : " · All questions answered"}</span></div><div class="readiness-coverage-track" role="progressbar" aria-label="Assessment completion" aria-valuenow="${coverage}" aria-valuemin="0" aria-valuemax="100"><span style="width:${coverage}%"></span></div>${missing ? `<button class="btn secondary small" data-action="review" type="button">Complete assessment</button>` : `<span class="readiness-coverage-complete">Assessment complete</span>`}</section>
      <section class="readiness-overview ${tone}">
        <div class="readiness-gauge-block">
          ...
          <strong>${missing ? "Provisional readiness" : "Readiness score"}</strong><span class="readiness-caption">Based on ${answered.length} answered question${answered.length === 1 ? "" : "s"}, not a probability of a breach</span>
        </div>
        <div class="readiness-overview-copy"><p class="eyebrow">${!answered.length ? "Answers needed" : counts[0] ? "Action needed" : findings.length ? "Room to improve" : "Keep the momentum"}</p><h2>${heading}</h2><p>${counts[0] ? "Start with the immediate items below. Delaying a weak safeguard leaves client data and daily operations exposed for longer." : findings.length ? "Use the priorities below to turn partial safeguards into consistent protection." : "Confirm your answers with evidence and review your safeguards when your firm changes."}</p>
          ...
          ${counts[0] ? `<div class="readiness-act-now"><span>Recommended this week</span><strong>Start with ${escapeHtml(findings[0].area)}. Assign an owner and verify the safeguard works.</strong></div>` : ""}
      ...
      <section class="readiness-panel readiness-priority-panel"><div class="readiness-panel-head"><p class="eyebrow">Priority actions</p><h2>${findings.length ? "Start with these safeguards" : "Maintain the safeguards you reported"}</h2><p>${findings.length ? "Address these reported gaps first. Each action includes the answer behind it and a practical next step." : "No gaps were flagged in the answers provided. Keep evidence that your controls work."}</p></div><div class="readiness-actions">${findings.slice(0, 3).map((flag, index) => visualReadinessAction(flag, index)).join("")}</div>${findings.length > 3 ? `<details class="readiness-more"><summary>View ${findings.length - 3} more actions</summary><div class="readiness-actions">${findings.slice(3).map((flag, index) => visualReadinessAction(flag, index + 3)).join("")}</div></details>` : ""}</section>
      <div class="readiness-chart-grid">
        <section class="readiness-panel"><div class="readiness-panel-head"><p class="eyebrow">Security areas</p><h2>Where your safeguards are weakest</h2><p>Assessed areas first, lowest scores at the top. Unassessed areas remain unknown.</p></div>
        ...
        <section class="readiness-panel"><div class="readiness-panel-head"><p class="eyebrow">Every safeguard, accounted for</p><h2>Your assessment at a glance</h2><p>Each square represents one question. Gray means unknown, not protected.</p></div><div class="readiness-control-map" role="img" aria-label="${counts[0]} immediate, ${counts[1]} within 30 days, ${counts[2]} within 90 days, ${strong} with no gap flagged, ${missing} unanswered">${segments.flatMap(item => Array.from({ length: item.count }, () => `<span class="${item.tone}" aria-hidden="true"></span>`)).join("")}</div><div class="readiness-legend">${segments.map(item => `<div><span class="readiness-dot ${item.tone}" aria-hidden="true"></span><span>${item.label}</span><strong>${item.count}</strong></div>`).join("")}</div><div class="readiness-explainer"><strong>${missing ? `${missing} safeguards still need assessment` : "All questions reviewed"}</strong><p>${missing ? "Complete the unanswered questions before treating this as a full picture of your firm." : "Validate the reported safeguards with evidence and revisit them when your firm changes."}</p></div></section>
      </div>
...
function visualReadinessAction(flag, index) {
  const impacts = {
    "Data Security": "Client information may be exposed through weak access or account controls.",
    "Backup & Recovery": "Data loss or an outage could interrupt work if recovery is not reliable.",
    "Tax Software & Cloud": "Disruption to core software or email could delay client work.",
    "Remote Access": "Unmanaged devices or remote connections can provide a route into firm systems.",
    "Compliance": "Undocumented responsibilities and inconsistent training can leave safeguards unimplemented.",
    "IT Support": "Gaps in maintenance and oversight can leave security issues unresolved.",
  };
  return `<details class="readiness-action" ${index < 3 ? "open" : ""}><summary><span class="readiness-action-number">${String(index + 1).padStart(2, "0")}</span><span class="readiness-action-title"><small>${escapeHtml(flag.area)}</small><strong>${escapeHtml(sections[flag.sectionIndex] || flag.title)}</strong></span><span class="severity ${flag.priority === "Immediate" ? "high" : "medium"}">${escapeHtml(flag.priority)}</span><span class="readiness-expand" aria-hidden="true">+</span></summary><div class="readiness-action-body"><p class="readiness-answer-evidence"><strong>You reported</strong><br>${escapeHtml(state.form[`question_${flag.sectionIndex}`] || "Not provided")}</p><p><strong>Why it matters</strong><br>${escapeHtml(impacts[flag.area] || "An incomplete safeguard can leave a gap in your firm's protection.")}</p><p><strong>Next step</strong><br>${escapeHtml(flag.fix)}</p><button class="btn secondary small" data-edit-section="${flag.sectionIndex}" type="button">Review answer</button></div></details>`;
}
...
function renderPublicAcknowledgement() {
  ...
  if (model.error && !request) {
    document.body.innerHTML = `<main class="public-ack-page public-ack-state-page"><header class="public-ack-state-header"><span>EasyWISP</span></header><section class="public-ack-state public-ack-state--unavailable"><p class="public-ack-kicker">Acknowledgement link</p><h1>This link can no longer be used.</h1><p class="public-ack-state-lead">${escapeHtml(model.error)}</p><div class="public-ack-state-note"><strong>Need a new link?</strong><span>Please contact the person who sent this acknowledgement to you.</span></div></section></main>`;
    return;
  }
  if (!request) {
    document.body.innerHTML = `<main class="public-ack-page"><section class="public-ack-state"><p class="public-ack-kicker">Signing request</p><h1>Loading secure signing request</h1><p>Please wait while we verify your link.</p></section></main>`;
    return;
  }
  if (model.completed) {
    document.body.innerHTML = `<main class="public-ack-page public-ack-state-page"><header class="public-ack-state-header"><span>EasyWISP</span></header><section class="public-ack-state public-ack-state--success"><p class="public-ack-kicker">Acknowledgement complete</p><h1>You are all set, ${escapeHtml(request.recipient_name || "there")}.</h1><p class="public-ack-state-lead">Your acknowledgement has been recorded for this Written Information Security Plan.</p><div class="public-ack-state-note"><strong>You can close this tab.</strong><span>Your firm now has a record of your acknowledgement.</span></div></section></main>`;
    return;
  }
  const snapshot = request.wisp_snapshot || {};
