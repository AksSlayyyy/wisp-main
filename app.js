  ...
  return `
    <div class="rr-report">
      ...
      <section class="rr-metrics" aria-label="Assessment summary">
        ...
        <div class="rr-metric"><span>No gap flagged</span><div class="rr-metric-value"><strong>${strong}</strong><small>of ${answered.length} assessed</small></div><p>Validate these controls with evidence</p></div>
      ...
      <div class="rr-analysis">
        <section class="rr-panel rr-domains"><div class="rr-section-head"><div><h2>Readiness by security area</h2><p>Lowest assessed scores first</p></div><span class="rr-unit">Score / 100</span></div>
          ...
          <div class="rr-chart-note"><span><i></i> Unassessed areas have no score</span><span>Self-reported · Higher is stronger</span></div>
        </section>
        <section class="rr-panel rr-priorities"><div class="rr-section-head"><div><h2>Action priorities</h2><p>Findings by suggested action window</p></div></div>
          <div class="rr-priority-bars">${segments.slice(0, 3).map(item => `<div class="rr-priority-row"><div><span>${item.label}</span><strong>${item.count}</strong></div><div class="rr-priority-track ${item.tone}"><i style="width:${findings.length ? item.count / findings.length * 100 : 0}%"></i></div></div>`).join("")}</div>
          <div class="rr-next-step"><span>${counts[0] ? "Recommended this week" : findings.length ? "Next step" : "Review status"}</span><p>${counts[0] ? `Prioritize ${escapeHtml(findings[0].area)}. Assign an owner to the immediate findings below.` : findings.length ? "Assign an owner and target date to each finding below." : answered.length ? "A strong starting point. Keep evidence that your safeguards work." : "Complete the assessment to identify your priorities."}</p></div>
        </section>
      </div>
      <section class="rr-panel rr-findings"><div class="rr-section-head"><div><h2>Findings <span class="rr-count">${findings.length}</span></h2><p>Review the reported gap, then expand for the recommended action.</p></div><span class="rr-unit">Ordered by priority</span></div>
        ${findings.length ? `<div class="rr-findings-labels" aria-hidden="true"><span>Safeguard / reported answer</span><span>Priority</span><span></span></div><div class="rr-finding-list">${findings.map((flag, index) => visualReadinessAction(flag, index)).join("")}</div>` : `<div class="rr-empty">${answered.length ? "No gaps flagged in your submitted answers." : "Your findings will appear here once you answer the assessment."}</div>`}
      </section>
      <footer class="rr-footer"><details><summary>Scoring methodology</summary><p>Readiness averages each answered question's score relative to its highest available option. Area scores use the same calculation. Unanswered questions are excluded.</p><p>Scores of 0–25 receive immediate priority, above 25–55 receive a 30-day priority, and above 55–75 receive a 90-day priority. These are suggested action windows, not legal deadlines. This report is based on your answers and is not an independent audit or compliance certification.</p></details><button type="button" data-action="view-summary">View submitted answers <span aria-hidden="true">↗</span></button></footer>
    </div>`;
}
function visualReadinessAction(flag, index) {
  ...
  return `<details class="rr-finding"><summary><span class="rr-finding-main"><strong>${escapeHtml(sections[flag.sectionIndex] || flag.title)}</strong><span>${escapeHtml(flag.area)} <span aria-hidden="true">·</span> ${escapeHtml(state.form[`question_${flag.sectionIndex}`] || "Not provided")}</span></span><span class="rr-priority ${flag.priority === "Immediate" ? "urgent" : flag.priority === "30 days" ? "attention" : "planned"}">${escapeHtml(flag.priority)}</span><span class="rr-chevron" aria-hidden="true">⌄</span></summary><div class="rr-finding-detail"><div><h3>Why it matters</h3><p>${escapeHtml(impacts[flag.area] || "An incomplete safeguard can leave a gap in your firm\'s protection.")}</p></div><div><h3>Recommended action</h3><p>${escapeHtml(flag.fix)}</p><button class="btn secondary small" data-edit-section="${flag.sectionIndex}" type="button">Review answer</button></div></div></details>`;
}
