...
create table if not exists public.wisp_acknowledgement_requests (
  ...
  wisp_snapshot jsonb not null default '{}'::jsonb,
...
create index if not exists wisp_acknowledgement_requests_project_status_idx
  on public.wisp_acknowledgement_requests(project_id, status, created_at desc);
create index if not exists wisp_acknowledgement_requests_staff_idx
  on public.wisp_acknowledgement_requests(firm_id, recipient_staff_id, created_at desc);
create unique index if not exists wisp_acknowledgement_requests_pending_recipient_idx
  on public.wisp_acknowledgement_requests(project_id, recipient_staff_id)
  where status = 'pending';

alter table public.wisp_acknowledgement_requests disable row level security;
...
grant execute on function public.create_wisp_acknowledgement_requests(uuid, jsonb, jsonb, text, integer) to anon, authenticated;
grant execute on function public.get_wisp_acknowledgement_request(uuid, text) to anon, authenticated;
grant execute on function public.complete_wisp_acknowledgement_request(uuid, text, text, text, text) to anon, authenticated;
grant execute on function public.list_wisp_acknowledgement_requests(uuid) to anon, authenticated;
