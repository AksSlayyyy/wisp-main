-- TESTING ONLY: the linked database has no firm_memberships table and this
-- workspace is intentionally operating without authenticated firm accounts.
-- Replace this migration with firm-membership authorization before production.
...
begin
  ...
  for v_recipient in select value from jsonb_array_elements(p_recipients) loop
    ...
    if exists (
      select 1 from public.wisp_acknowledgement_requests
      where project_id = p_project_id and recipient_staff_id = v_staff_id and status = 'signed'
    ) then
    ...
    insert into public.wisp_acknowledgement_requests (
      project_id, firm_id, recipient_staff_id, recipient_name, recipient_email,
      recipient_role, acknowledgement_text, wisp_snapshot, access_token_hash, expires_at
    ) values (
...
grant execute on function public.remove_wisp_acknowledgement_request(uuid, uuid) to anon, authenticated;